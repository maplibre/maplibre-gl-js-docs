...
class TableOfContentsNote extends React.Component {
    render() {
        ...
        return (
            <li className="mt24">
                <a className="txt-bold link link--gray" href={href(doc)}>
                    {doc.name}
                </a>
            </li>
        );
...
class TableOfContentsItem extends React.Component {
    ...
    render() {
        ...
        if (
            empty(doc.members.static) &&
            empty(doc.members.instance) &&
            empty(doc.members.events)
        ) {
            return (
                <li className="txt-s mt6">
                    <a href={href(doc)} className="link--gray block">
                        {doc.name}
                    </a>
                </li>
            );
        ...
        const membersList = (members, title, sigil) =>
            ...
            members.length !== 0 && (
                <ul className="list-reset px6 mb6">
                    ...
                    {members.map((m, i) => (
                        <li key={i} className="ml12">
                            <a
                                href={href(m)}
                                className="block txt-s truncate pre-open link--gray"
                            >
                                {sigil}
                                {m.name}
                            </a>
            ...
            <li>
                ...
                >
                    ...
                    <Icon
                        inline={true}
                        name={`${
                            this.state.disclosed ? 'caret-down' : 'caret-right'
                        }`}
                    />
                </a>
