...
function redirect(location) {
    ...
    if (expressionExceptions.indexOf(location.hash.replace('#', '')) > -1) {
        return `${pathRoot}/style-spec/expressions/${location.hash}`;
    }
    ...
    if (location.hash === '#other-filter') {
        return `${pathRoot}/style-spec/other/#other-filter`;
    }
    ...
    if (
        ...
        return `${pathRoot}/style-spec/layers/${location.hash}`;
    }
    ...
    hash.splice(0, 1);
    return `${pathRoot}/style-spec/${page}/${
